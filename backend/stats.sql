...
SELECT
  ...
  ROUND(AVG(opts_por_pregunta), 1) AS promedio_por_pregunta,
...
SELECT
  ...
  ROUND(
    AVG(correct_rate) FILTER (WHERE times_answered > 0) * 100, 1
  )                                                             AS pct_acierto_promedio,
  ROUND(
    AVG(average_response_time_ms) FILTER (WHERE times_answered > 0) / 1000.0, 1
...
SELECT
  ...
  ROUND(qa.correct_rate * 100, 1)       AS pct_acierto,
  ROUND(qa.average_response_time_ms / 1000.0, 1) AS seg_promedio
...
-- ── 11. TOP 10 PREGUNTAS MÁS DIFÍCILES (mín. 5 respuestas) ───
SELECT
  tr.title,
  ROUND(qa.correct_rate * 100, 1)       AS pct_acierto,
  qa.times_answered                     AS respondidas,
...
-- ── 12. TOP 10 PREGUNTAS MÁS FÁCILES (mín. 5 respuestas) ─────
SELECT
  tr.title,
  ROUND(qa.correct_rate * 100, 1)       AS pct_acierto,
  qa.times_answered                     AS respondidas
...
-- ── 13. PREGUNTAS QUE NADIE RESPONDIÓ BIEN (correct_rate = 0) ─
SELECT
  tr.title,
  qa.times_answered AS respondidas
FROM      question_analytics qa
